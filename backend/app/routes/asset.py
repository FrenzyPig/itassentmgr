        ...
        for index, row in df.iterrows():
            try:
                ...
                data = {
                    ...
                    'device_config': str(row['设备配置']).strip() if '设备配置' in df.columns and pd.notna(row['设备配置']) and str(row['设备配置']) != 'nan' else None,
                    'software_info': str(row['软件信息']).strip() if '软件信息' in df.columns and pd.notna(row['软件信息']) and str(row['软件信息']) != 'nan' else None,
                    'location_info': str(row['位置信息']).strip() if '位置信息' in df.columns and pd.notna(row['位置信息']) and str(row['位置信息']) != 'nan' else None,
