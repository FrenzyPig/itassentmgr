        ...
        <el-form-item v-if="currentMacs.length > 0" label="MAC地址">
          <div style="display: flex; flex-direction: column;">
            <div v-for="mac in currentMacs" :key="mac.id" class="mac-row">
              ...
              <el-input v-model="mac.ip" placeholder="IP地址(默认0.0.0.0)" style="width: 220px" />
            </div>
          </div>
        </el-form-item>

        <el-form-item label="领用人" prop="user_name">
          <el-input v-model="form.user_name" placeholder="请输入领用人姓名" />
        </el-form-item>
...
const rules = {
  ...
  user_name: [{ required: true, message: '请输入领用人', trigger: 'blur' }]
}
...
async function fetchAvailableAssets() {
  try {
    const res: any = await assetApi.getList({ status: '在库', pageSize: 100 })
    if (res.code === 200) {
...
async function handleSubmit() {
  ...
  try {
    await store.claimAsset(form.asset_id, {
